...
![Cases](https://img.shields.io/badge/Cases-110-blue)
...
├── tests/                        测试用例（110 个 / 102 函数，按 marker 分层）
...
42 个 unit 用例正常跑，68 个环境层用例等账号激活。
...
# 3. 跑测试
python scripts/run_tests.py -m "unit and not platform"  # 冒烟 35 个，不等账号秒出
python scripts/run_tests.py -m unit                     # unit 层 42 个（含 7 个 Web 层）
python scripts/run_tests.py                             # 全量 110 用例 + HTML 报告
